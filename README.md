...
当前版本为 **v0.8.1**。它在原生 SwiftUI 产品外壳和 CEF/Chromium ARM64 运行时之上提供 SQLite 会话、垂直标签与工作空间、仅左右双页面分屏、精选域名目录隐私盾牌、权限、隐私窗口和下载管理。v0.8.1 为历史记录加入按过去 1 小时、24 小时、7 天或所有时间永久删除的入口，收紧收藏与下载空状态的顶部间距，并把 macOS 标题栏拆为交通灯/性能监控左卡片与导航右卡片。Swift Package 保留 WebKit 预览构建，真实 Chromium 构建使用生成的 Xcode 工程。
