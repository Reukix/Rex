...
当前版本为 **v0.9.8 build 982 Beta**。Rex 的 SwiftUI/AppKit 层拥有用户能看到的浏览器外壳、导航、标签、右侧下载模块和 `rex://extensions` 扩展管理界面；CEF/Chromium 是网页、导航、下载、扩展执行、权限和配置的权威后端。下载请求、重定向、传输、落盘与生命周期全部由 Chromium 处理，Rex 只映射其 URL、原始 URL、文件名、MIME、字节数、进度百分比、路径、状态和终态；Rex 不预先推断生命周期。Chromium 原生保存/下载浮层和开始动画保持关闭，文件默认保存到 `~/Downloads/Rex`，新下载开始时自动弹出 Rex 下载浮层。Swift Package 保留预览构建，真实 Chromium 构建使用生成的 Xcode 工程。
...
Scripts/package-chromium-app.sh 0.9.8 982 Release
# 产物：Dist/Rex.app 与 Dist/Rex-v0.9.8-macos-arm64-chromium.zip
```

build 982 的本地 Beta 包已经生成：`Dist/Rex.app`（344M）与
`Dist/Rex-v0.9.8-macos-arm64-chromium.zip`（143,034,126 bytes / 145M），ZIP
SHA-256 为 `d57573e5b5bc441e37ad07270f9e90cb4f7aafd53cc8e8d8f65f7ad372fa2e8e`。
