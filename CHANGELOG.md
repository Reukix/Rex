...
- 顶栏重新设计为贯穿窗口宽度的连续系统材质带：红黄绿按钮与导航控件共用同一背景，只让控件内容避让窗口按钮，并用底部分隔线连接侧栏和网页区域，消除按钮下方的裸露空白。
...
- 产物：`Dist/Rex-v0.8.1-macos-arm64-chromium.zip`（143,352,645 bytes；`du` 约 144M）/ `Dist/Rex.app`（约 343M）。
- ZIP SHA-256：`279fedf53f5f1ea38e72fac053e161b47d7dd8dbc39c280d0b1d72d570e90b5f`；压缩数据完整性校验通过。
- 主程序、CEF 与五个 Helper 均为纯 arm64，App/Helper 版本均为 `0.8.1 / 810`。真实 App 冒烟确认连续标题栏、侧栏展开/折叠及全屏往返正常；四档历史删除菜单、收藏/下载空状态的上一轮验证继续有效，运行中 CEF 子进程使用 `Rex/0.8.1`。
