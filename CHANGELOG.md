...
- 主窗口启用全尺寸内容标题栏，并恢复为 50 pt 槽内的 44 pt 单导航卡：普通窗口将红黄绿按钮下移 4 pt，再根据按钮真实位置动态避让，导航卡从其右侧开始；全屏时使用 8 pt 普通边距。
...
- 产物：`Dist/Rex-v0.8.1-macos-arm64-chromium.zip`（143,463,122 bytes；`du` 约 144M）/ `Dist/Rex.app`（约 344M）。
- ZIP SHA-256：`b60aa1a236a2189e39f6483d40cfe6abf6e5969df5b75aaa8888beb4962d3174`；压缩数据完整性校验通过。
