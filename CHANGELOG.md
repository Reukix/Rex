...
- 主窗口启用全尺寸内容标题栏，并恢复为 50 pt 槽内的 44 pt 单导航卡：普通窗口根据真实红黄绿窗口按钮位置动态避让，导航卡从其右侧开始；全屏时使用 8 pt 普通边距。
- 内存/CPU 指标回到导航栏首项，点击仍可打开性能监测详情页；红黄绿按钮周围的无控件区域保留为窗口拖动区。
- 性能详情页显示 Rex 总内存与总 CPU，以及每个标签页的标题、站点、生命周期、CEF 任务内存和 CPU。移除 CPU/内存排序切换和行分隔线，改为按标签页顺序稳定显示的独立网页卡片。
...
- 产物：`Dist/Rex-v0.8.1-macos-arm64-chromium.zip`（143,460,176 bytes；`du` 约 144M）/ `Dist/Rex.app`（约 344M）。
- ZIP SHA-256：`1b91bb346091be351c3ded71223ffb26b35923b395102696ea79bf4c4c2bdc51`；压缩数据完整性校验通过。
- 主程序、CEF 与五个 Helper 均为纯 arm64，App/Helper 版本均为 `0.8.1 / 810`。真实 App 冒烟确认紧凑单导航卡与性能网页卡片无覆盖或断层；详情页成功读取真实网页任务指标并正确保留无数据标签。四档历史删除菜单、收藏/下载空状态的上一轮验证继续有效，运行中 CEF 子进程使用 `Rex/0.8.1`。
