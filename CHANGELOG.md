...
- 产物：`Dist/Rex-v0.8.1-macos-arm64-chromium.zip`（143,345,876 bytes；`du` 约 145M）/ `Dist/Rex.app`（约 343M）。
- ZIP SHA-256：`4dab0b2168e8210683727866a2c3a6b0d871ea98223978bf9ed5ce82debe8724`；压缩数据完整性校验通过。
- 主程序、CEF 与五个 Helper 均为纯 arm64，App/Helper 版本均为 `0.8.1 / 810`。真实 App 冒烟确认标题栏、全屏回退、四档历史删除菜单及确认框、收藏/下载空状态正常，运行中 CEF 子进程使用 `Rex/0.8.1`。
