...
rex_request_normal_termination() {
  ...
  if rex_quit_output="$(/usr/bin/osascript -l JavaScript -e '
    function run(argv) {
      Application(argv[0]).quit()
    }
  ' "$2" 2>&1)"; then
    return 0
  fi
  # Rex intentionally cancels Cocoa's default termination transaction, then
  # stops NSApplication.run after Chromium closes. Apple Events surface that
  # handoff as userCanceledErr even though the process exits normally.
  if [[ "$rex_quit_output" == *"(-128)"* ]]; then
    return 0
  fi
...
rex_cleanup() {
  ...
  if [[ -n "$rex_real_fingerprint_before" ]]; then
    rex_real_fingerprint_after="$(rex_real_state_fingerprint)"
...
rex_real_fingerprint_before="$(rex_real_state_fingerprint)"
...
CFFIXED_USER_HOME="$rex_smoke_home" \
  "$rex_executable" >"$rex_smoke_log" 2>&1 &
rex_main_pid=$!
...
if ! rex_request_normal_termination "$rex_main_pid" "$rex_app_path"; then
