...
5. Chromium 下载权威边界、隐私目录更新、站点归属判断和隐私窗口落盘语义具有明确且经过测试的安全边界。
...
- native messaging 的通用能力有独立结论；iCloud Passwords 只有获得 Apple managed entitlement 或被 helper 的签名名单接纳，且原始 helper 与正式签名 Rex 端到端通过时才可标记支持；当前个人团队 Apple Development ZIP 必须显示受限。
...
| 下载威胁检测缺失 | 保持 Chromium 单一传输链；另行评估可审计且不接管生命周期的信誉/内容检测 | 不得以 Rex 本地启发式冒充 Safe Browsing 或恶意内容扫描 |
| 隐私窗口下载会落盘 | 只阻止普通资料库记录，不能宣称无磁盘痕迹 | 关闭隐私窗口后无普通历史/下载记录，但 Chromium 保存的文件仍可存在 |
