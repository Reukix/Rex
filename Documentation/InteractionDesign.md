...
Rex 扩展页分为「发现」与「已导入」。发现页只展示有真实 Chrome Web Store ID 的精选扩展，支持本地搜索和推荐/隐私/效率/阅读/开发分类；官方详情和完整搜索在系统默认浏览器打开。已导入页只管理用户选择的 Manifest V2/V3 文件夹副本，并明确标注「仅保存包，不执行扩展代码」；在运行时不可用时不显示没有实际效果的启用开关。界面必须持续说明 CEF 150 不会安装 Chrome Web Store 包，也不会执行本地扩展脚本。
