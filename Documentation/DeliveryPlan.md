...
建立带稳定 accessibility identifier 的测试：新建/关闭/恢复标签、空间切换、键盘聚焦地址栏、左右分屏、标签右键菜单、拖动比例、交换页面、隐私盾牌、降低透明度和完整键盘遍历。v0.9.0 额外覆盖新标签页等宽卡片、搜索引擎图标、收藏名称/网址表单校验、关于页版本元数据、中文主菜单和扩展目录官方链接。Chromium 集成测试使用本地受控测试站点，不依赖公网。
...
| 扩展兼容范围 | 用户预期落差 | 商店目录、本地包管理与运行时分层表达，不宣传直接安装或完整兼容 |
